...
import { CreateShoppingListItemSchema } from '../../../../spec/schemas';
...
export default defineEventHandler(async (event) => {
  ...
  const validation = CreateShoppingListItemSchema.safeParse(data);
  ...
  const item = await shoppingListRepo.addItem(validation.data);

  setResponseStatus(event, 201);
  
  // Build response based on whether it's an ingredient or article
  const baseResponse = {
    id: item.id,
